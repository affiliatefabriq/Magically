...
import { X, Maximize2, Share2, Download, Heart, MessageCircle, Check, Dot } from "lucide-react";
...
export const PublicationDialog = ({ publication, children }: { publication: Publication, children: React.ReactNode }) => {
    ...
    const goFullScreen = () => {
        const elem = document.querySelector(".publication-fullscreen-target") as HTMLElement;
        if (!elem) return;
    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent
                className="flex theme w-full h-full max-w-7xl max-h-[90vh] p-0 m-0 overflow-hidden "
            >
                <DialogTitle className="sr-only">Publication by {publication.author.username}</DialogTitle>
                ...
                <div className="relative w-full h-full flex-1 publication-fullscreen-target">
                    ...
                    >
                        {isNativeFullscreen ? <X className="size-6" /> : <Maximize2 className="size-6" />}
                    </button>
                </div>
                ...
                <div className="relative w-full flex flex-1 flex-col h-screen md:h-full border-t md:border-t-0 md:border-l overflow-hidden">
                    <div className="flex flex-col items-start justify-center p-4 border-b gap-3 shrink-0 bg-background/95 backdrop-blur supports-backdrop-filter:bg-background/60">
                        <Link href={`/profile/${publication.author.username}`} className="flex items-center gap-2">
                            <UserAvatar {...publication.author} size="sm" />
                            <span className="font-semibold text-sm truncate">{publication.author.username}</span>
                            <Dot />
